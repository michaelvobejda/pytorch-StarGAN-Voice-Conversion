...
def convert(src_wav_file, trg_spk, convert_dir, model_save_dir, resume_iters):

    os.makedirs(join(convert_dir, str(resume_iters)), exist_ok=True)
    sampling_rate, num_mcep, frame_period = 16000, 36, 5
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # test_wavfiles = test_loader.get_batch_test_data(batch_size=config.num_converted_wavs)
    # test_wavs = [load_wav(wavfile, sampling_rate) for wavfile in test_wavfiles]
    src_wav = load_wav(src_wav_file, sampling_rate)
    #src_wav_base_name = src_wav_file.split('/')[-1]
    wav_name = src_wav_file.split('/')[-1]
    wav_id = wav_name.split('.')[0]
    #wav_name = src_wav_base_name.split('.')
    #wav_id = src_spk
    #trg_wav = load_wav(trg_wav_file, sampling_rate)
    ...
    with torch.no_grad():
        ...
        #     mean_log_src=test_loader.logf0s_mean_src, std_log_src=test_loader.logf0s_std_src,
        #     mean_log_target=test_loader.logf0s_mean_trg, std_log_target=test_loader.logf0s_std_trg)
        f0, timeaxis, sp, ap = world_decompose(wav=src_wav, fs=sampling_rate, frame_period=frame_period)
        coded_sp = world_encode_spectral_envelop(sp=sp, fs=sampling_rate, dim=num_mcep)

        #trg_f0, trg_timeaxis, trg_sp, trg_ap = world_decompose(wav=src_wav, fs=sampling_rate, frame_period=frame_period)

        trg_stats_path = 'data/concatted_audio/mc/' + trg_spk + '_stats.npz'
        trg_stats = np.load(trg_stats_path)

        # logf0s_mean_src = np.mean(np.log(f0))
        # logf0s_std_src = np.std(np.log(f0))

        # TODO: wrap f0s in list [f0s]?
        logf0s_mean_src, logf0s_std_src = logf0_statistics([f0])

        mcep_mean_src = coded_sp.mean(axis=0)
        mcep_std_src = coded_sp.std(axis=0)

        logf0s_mean_trg = trg_stats['log_f0s_mean']
        logf0s_std_trg = trg_stats['log_f0s_std']

        mcep_mean_trg = trg_stats['coded_sps_mean']
        mcep_std_trg = trg_stats['coded_sps_std']
        ...
        coded_sp_norm_tensor = torch.FloatTensor(coded_sp_norm.T).unsqueeze_(0).unsqueeze_(1).to(device)
        #coded_sp_tensor = torch.FloatTensor(coded_sp.T).unsqueeze_(0).unsqueeze_(1).to(device)
        #spk_conds = torch.FloatTensor(test_loader.spk_c_trg).to(device)
        # print(spk_conds.size())
        # coded_sp_converted_norm = G(coded_sp_norm_tensor, spk_conds).data.cpu().numpy()
        ...
        trg_mc = spk_name_to_mc(trg_spk)
        trg_mc = torch.FloatTensor(trg_mc).to(device)
        trg_mc.unsqueeze_(0)
        trg_mc.unsqueeze_(0)
        trg_mc.transpose_(2, 3)

        # coded_sp_converted = G(coded_sp_norm_tensor, trg_mc).data.cpu().numpy()

        #coded_sp_converted_norm = G(coded_sp_tensor, spk_conds).data.cpu().numpy()
        coded_sp_converted_norm = G(coded_sp_norm_tensor, trg_mc).data.cpu().numpy()
        #coded_sp_converted = np.squeeze(coded_sp_converted_norm).T * test_loader.mcep_std_trg + test_loader.mcep_mean_trg
        coded_sp_converted = np.squeeze(coded_sp_converted_norm).T * mcep_std_trg + mcep_mean_trg
        # coded_sp_converted = np.ascontiguousarray(coded_sp_converted)
        #coded_sp_converted = np.squeeze(coded_sp_converted).T
        coded_sp_converted = np.ascontiguousarray(coded_sp_converted)
        ...
        #                                          ap=ap, fs=sampling_rate, frame_period=frame_period)
        # wav_id = wav_name.split('.')[0]
        print(f'Saving to {wav_id}-vcto-{trg_spk}.wav')
        librosa.output.write_wav(join(convert_dir, str(resume_iters),
            f'{wav_id}-vcto-{trg_spk}.wav'), wav_transformed, sampling_rate)
        if True:
            wav_cpsyn = world_speech_synthesis(f0=f0, coded_sp=coded_sp,
                                            ap=ap, fs=sampling_rate, frame_period=frame_period)
...
model_save_dir = './models/20190328_first_run/'
convert_dir = './converted/20190328_first_run/'
resume_iters = 23000
convert(src_wav_file='data/VCTK-Corpus/wav16/p302/p302_027.wav', 
        #src_spk='p229',
        #trg_wav_file='data/concatted_audio/wav/p225/p225_concatted_0.wav', 
        trg_spk='p300',
